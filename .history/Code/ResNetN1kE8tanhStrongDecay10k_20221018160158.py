...
Date           : 2022-05-09 18:28:28
Last Revision  : 2022-05-09 18:29:17
Last Author    : Jie Li
File Path      : /AI-assisstedChangePointDetection/Python/ResNetN1kE8tanhStrongDecay10k.py
Description    : based on ResNetN1kE8tanhStrong with decay 10k
...
# %% normalization
data_x = Transform2Dtanh(data_x)
data_x = Standardize(data_x)
indnan0 = np.isnan(data_x).any(axis=2)
indnan1 = ~indnan0.any(axis=1)
data_x = data_x[indnan1, :, :]
...
print(current_file)
model_name = current_file.replace('ResNetWeak', '')
...
current_file = Path(__file__).stem
print(current_file)
logdir = pathlib.Path.cwd() / "tensorboard_logs" / "Trial"
model_name = 'ResNetN1kE8tanh'
# %%
num_classes = 5
# load model for the folder
model_path = logdir / model_name / 'model'
