...
Last Revision  : 2023-09-20 13:24:51
Last Author    : Jie Li
...
from autocpd.neuralnetwork import compile_and_fit, deep_nn
from autocpd.utils import GenSetGen, Transform2D2TR
...
data_x = np.delete(data_x, np.arange(2 * N_sub, 4 * N_sub), 0)
...
cp_all = np.concatenate((cp_non, cp_mean, cp_slope))
ind_range = np.arange(N_sub * num_dataset)
(
    x_train,
    x_test,
    y_train,
    ...
    ind_train,
    ind_test,
) = train_test_split(data_x, data_y, cp_all, ind_range, train_size=0.8, random_state=42)
...
# %%
model_name = current_file + "n" + str(n) + "N" + str(N_sub) + "L" + str(21)
print(model_name)
# build the model
