...
Date           : 2022-11-04 23:49:26
Last Revision  : 2022-11-06 10:17:57
Last Author    : Jie Li
File Path      : /AutoCPD/Code/ScenarioS2RhoRandomL5Predict.py
Description    :
...
result_nnL5 = np.empty((num_models, num_repeat, 5))
for i in range(num_models):
	...
	modelL5 = tf.keras.models.load_model(model_path)
	...
	for j in range(num_repeat):
		...
		result_nnL5[i, j, 0] = mer_nnL5
		result_nnL5[i, j, 1:] = np.reshape(confusion_mtx, (4,))

# %%
# save the cusum threshold to folder datasets/CusumResult/
nnL5_vec = np.mean(result_nnL5, axis=1, keepdims=False)[:, 0]
path_nnL5 = Path(cusum_result_folder, current_file + "result_nnL5")
