...
Date           : 2022-11-05 00:11:11
Last Revision  : 2022-11-06 10:17:24
Last Author    : Jie Li
File Path      : /AutoCPD/Code/ScenarioS2RhoRandomL5Train.py
Description    :
...
m_vec = 4 * Q  # only for pruned cusum
...
for i in range(n_len):
	...
	for j in range(num_N):
		...
		m = m_vec[i]
		model_name = current_file + "n" + str(n) + "N" + str(2 * N
															) + "m" + str(m)
		print(model_name)
		# build the model
		input = layers.Input(shape=(n,), name="Input")
		x = layers.Dense(m, activation="relu", kernel_regularizer='l2')(input)
		x = layers.Dense(m, activation="relu", kernel_regularizer='l2')(x)
		x = layers.Dense(m, activation="relu", kernel_regularizer='l2')(x)
		x = layers.Dense(m, activation="relu", kernel_regularizer='l2')(x)
		x = layers.Dense(m, activation="relu", kernel_regularizer='l2')(x)
